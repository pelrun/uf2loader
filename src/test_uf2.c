...
// External functions from uf2_test.c
extern uint32_t crc32_calculate(const uint8_t *data, size_t size);
...
// Flash functions are provided by test_shared.c
#ifdef TEST_MODE
extern void flash_range_erase(uint32_t addr, size_t size);
extern void flash_range_program(uint32_t addr, const uint8_t *data, size_t size);
#endif
...
static void create_uf2_block(struct uf2_block *block, uint32_t block_no, uint32_t num_blocks,
                           uint32_t target_addr, const uint8_t *data, uint16_t family_id) {
    memset(block, 0, sizeof(struct uf2_block));
    
    // UF2 magic numbers
    block->magic_start0 = 0x0A324655;
    block->magic_start1 = 0x9E5D5157;
    block->magic_end = 0x0AB16F30;
    
    // Block info
    block->flags = 0x00002000;  // Family ID present
    block->target_addr = target_addr;
    block->payload_size = FLASH_PAGE_SIZE;
    block->block_no = block_no;
    block->num_blocks = num_blocks;
    block->file_size = family_id | (family_id << 16);  // Family ID in both halves
    
    // Copy data
    if (data) {
        memcpy(block->data, data, FLASH_PAGE_SIZE);
    }
}
...
TEST(block_sequence) {
    struct uf2_block block1, block2, block3;
